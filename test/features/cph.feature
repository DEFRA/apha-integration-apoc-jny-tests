...
Feature: (AIL-245) HOLDINGS endpoint tests
  ...
  Scenario Outline: 01 Verify that, Unauthorised response (401) should be returned for invalid credentials
  ...
  Scenario Outline: 02 Verify that a valid CPH number returns a successful response
  ...
  Scenario Outline: 03 Verify that, Unsuccessful response (404) should be returned for a non-existent CPH number
    Given the user submits a CPH request with CPH number "<cphNumber>"
    When the request is processed by the system
  ...
  Scenario Outline: 04 Verify that the appropriate error message is returned when a user supplies an invalid CPH number
    Given the user submits a CPH request with CPH number "<cphNumber>"
    When the request is processed by the system
